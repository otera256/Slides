...
$i$層目の重み行列を$W_i$,バイアスを$b_i$,活性化関数を$sigma$とおくと、例えば5層のMLPは以下のように定式化できる
$
  h_1 &= sigma(W_1 x + b_1)\
  ...
  y &= W_4 h_3 + b_4
$
ここで、$x$は入力、$y$は出力である。誤差伝搬においてバイアスは影響を与えないので省略すると$i$層目の線形層を$W_i$という関数として表記すると
$
  y = W_4 circle.small sigma circle.small W_3 circle.small sigma circle.small W_2 circle.small sigma circle.small W_1 (x)
$
逆伝搬の例として$pdv(L, h_1)$を求めると
$ pdv(L, h_1) = pdv(L, y) dot pdv(y, h_3) dot pdv(h_3, h_2) dot pdv(h_2, h_1) = pdv(L, y) dot W_4 dot Sigma_3 dot W_3 dot Sigma_2 dot W_2 $
となる。ここで$Sigma_i$は$i$層目の$sigma$の微分係数を対角成分に持つ対角行列である。

深層学習では$W$と$Sigma$の繰り返しが多くなっていくため、どこかで勾配の値が極端に大きくなるか小さくなると、勾配消失や勾配爆発が起きて学習がうまくいかなくなる。

$W$と$Sigma$の性質を調べることで、勾配消失・爆発を防ぐ初期化手法を導出できる。$W$は学習によって更新されていくので、初期値をうまく設定することが重要になる。$Sigma$は次の章で扱う。
...
#let xs = lq.linspace(-6, 6, num: 100)
...
#grid(
  ...
  align(center + horizon)[
    #lq.diagram(
      ...
      xlabel: $x$, ylabel: $y$,
      lq.plot(xs, xs.map(x => sigmoid(x)), mark: none, color: orange, label: "sigmoid"),
      legend: (position: bottom + right),
    )
  ],
  align(left + horizon)[
    $ pdv(sigma, x) = sigma(x)(1 - sigma(x)) $
    
    Sigmoid関数の勾配は、$x$ が大きく正または負のときに非常に小さくなる（0に近づく）。これが*勾配消失 (Vanishing Gradient)* の原因。

    $->$ 深いネットワークでは、勾配がどんどん小さくなり、学習が進まなくなる。
  ]
)
...
#grid(
  ...
  align(center + horizon)[
    #lq.diagram(
      height: 200pt, width: 300pt,
      xlabel: $x$, ylabel: $y$,
      lq.plot(xs, xs.map(x => tanh(x)), mark: none, color: orange, label: "tanh"),
      legend: (position: bottom + right),
    )
  ],
  align(left + horizon)[
    $ pdv(tanh, x) = 1 - tanh^2(x) $
    
    Tanh関数の勾配は、Sigmoidに比べて原点付近で大きい。これにより、勾配消失の問題が多少軽減される。

    また原点対称であることが、ニューラルネットワークの学習を安定化させる効果がある。
  ]
)
...
#grid(
  ...
  align(center + horizon)[
    #lq.diagram(
      height: 200pt, width: 300pt,
      xlabel: $x$, ylabel: $y$,
      lq.plot(xs, xs.map(x => relu(x)), mark: none, color: orange, label: "ReLU"),
      legend: (position: bottom + right),
    )
  ],
  align(left + horizon)[
    $ pdv("ReLU", x) = "if" (x > 0) { 1 } "else" { 0 } $
    
    ReLU関数の勾配は、正の入力に対して常に1である。これにより、勾配消失の問題が大幅に軽減される。

    $->$ 深いネットワークでも効果的に学習でき、計算も高速。最も広く使われている活性化関数。

    ただし$x = 0$のとき微分不可能であるのに加えて、負の入力に対して勾配が0になるという欠点もある。
  ]
)

== 適応型活性化関数

ReLUの欠点を補うために、学習可能なパラメータを持つ活性化関数も提案されている。

- *Leaky ReLU*: $f(x) = max(0.01x, x)$。負の入力に対しても微小な勾配を持つ。
- *Parametric ReLU (PReLU)*: $f(x) = max(alpha x, x)$。負の入力に対する勾配を学習可能なパラメータ $alpha$ で調整。
- *ELU (Exponential Linear Unit)*: $f(x) = x$ (x > 0), $alpha (exp(x) - 1)$ (x <= 0)。負の入力に対して滑らかな出力を持つ。

#pagebreak()

#let leaky_relu(x) = calc.max(0.01 * x, x)
#let prelu(x, alpha) = calc.max(alpha * x, x)
#let elu(x, alpha) = if (x > 0) { x } else { alpha * (calc.exp(x) - 1) }

#align(center + horizon)[
  #lq.diagram(
    height: 300pt, width: 600pt,
    xlabel: $x$, ylabel: $y$,
    lq.plot(xs, xs.map(x => leaky_relu(x)), mark: none, color: orange, label: "Leaky ReLU"),
    lq.plot(xs, xs.map(x => prelu(x, 0.25)), mark: none, color: green, label: "PReLU (alpha=0.25)"),
    lq.plot(xs, xs.map(x => elu(x, 1.0)), mark: none, color: blue, label: "ELU (alpha=1.0)"),
    legend: (position: bottom + right),
  )
]
...
#grid(
  ...
  align(center + horizon)[
    #lq.diagram(
      height: 200pt, width: 300pt,
      xlabel: $x$, ylabel: $y$,
      lq.plot(xs, xs.map(x => swish(x)), mark: none, color: orange, label: "Swish"),
      legend: (position: bottom + right),
    )
  ],
  align(left + horizon)[
    $ pdv(swish, x) = sigma(x) + x dot sigma(x)(1 - sigma(x)) $
    
    Swish関数は、ReLUよりも滑らかで、負の入力に対しても非ゼロの出力を持つ。これにより、いくつかのタスクでReLUを上回る性能を示すことが報告されている。
  ]
)

他にも多くの活性化関数が提案されており、ネットワークの種類のよって適した活性化関数も異なる
...
#align(center)[
  #image("images/overfitting.png", height: 250pt)
]
...
#align(center)[
  #image("images/dropout.png", height: 300pt)
  #text(size: 12pt, fill: gray)[
    出典: #link("https://jmlr.org/papers/v15/srivastava14a.html")[Dropout: A Simple Way to Prevent Neural Networks from Overfitting]
  ]
]
...
$
  mu = rho mu + (1 - rho) mu_B \
  sigma^2 = rho sigma^2 + (1 - rho) sigma_B^2
$
$rho$は移動平均の係数（通常0.9程度）。$mu_B, sigma_B$はミニバッチごとの平均・分散。
